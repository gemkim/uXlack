...
export default forwardRef<InputTextRefType, InputPropsType>(function InputText(
  ...
  const { maxWidth, defaultLine = 'line', lineColor = '#dbdbdb', focusColor = '#64748b' } = styleOpt
  ...
  return (
    <div
      className={cn(
        'relative w-full font-[16px] leading-none',
        className,
        isFocus && 'isFocus',
        isError && 'error'
      )}
    ...
    >
      <label>
        <input
          ...
          name={name}
          className={cn(
            'block w-full h-10 p-2.5 bg-transparent text-black outline-none transition-colors',
            lineClass,
            errorClass,
            focusClass,
